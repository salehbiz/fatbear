...
export default function BusinessStory({staticView=false}:{staticView?:boolean}){
 ...
 // Restore scroll-derived digits and chart after every React commit, including
 // development refresh at a stationary final frame. Static mode owns its own final UI.
 useLayoutEffect(()=>{
 ...
 return <section ref={sectionRef} className={`business-story${staticView?' business-static':''}`} aria-label="Maya’s illustrative creator business">
  ...
  <aside className="purchase-notifications" aria-label="Illustrative purchases">
   {['Photo diary','Behind the scenes','Poolside collection','Photo diary','Studio collection','Behind the scenes'].map((product,i)=><div className="purchase-notification" key={i}><img src={media(`inbox/${conversations[i].avatar}`)} alt=""/><div><header><span>Passes</span><small>now</small></header><strong>{conversations[i].name} bought {product.toLowerCase()}.</strong><p>Purchase confirmed <span>+$40</span></p></div></div>)}
  </aside>
  <div className="business-film"><div className="business-film-image"><img src={media(`business/${staticView?'end':'crew-poster'}.jpg`)} width="1280" height="720" alt="Maya’s photography crew working beside the pool"/>{!staticView&&<canvas width="1280" height="720" aria-hidden="true"/>}</div></div>
  ...
  <section className="business-dashboard" aria-label="Illustrative creator dashboard">
   <aside className="passes-sidebar" aria-label="Illustrative creator navigation">
    ...
    <div className="passes-sidebar-account"><img src={media('avatar.webp')} alt="Maya"/><span>Create</span><PlusCircle size={16}/></div>
   </aside>
   <div className="passes-workspace">
   <header className="dashboard-header"><div className="passes-tabs"><strong>Dashboard</strong><span>Explore</span></div><div className="dashboard-owner"><img src={media('avatar.webp')} alt="Maya"/><span>{creator.handle}<VerifiedBadge/></span></div></header>
