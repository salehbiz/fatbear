...
export default function InstagramPost({staticView=false}:{staticView?:boolean}) {
  ...
  return <article className="post-layer" aria-label="Maya's illustrative Instagram post">
    ...
    <header className="post-header"><span className="avatar-ring"><img src={media('avatar.webp')} width="34" height="34" loading="lazy" decoding="async" alt=""/></span><div><strong>{creator.handle}<VerifiedBadge/></strong><span>Original audio</span></div><DotsThree size={25} className="post-menu" aria-hidden="true"/></header>
    <div className="post-photograph">{staticView && <img className="static-post-photo" src={media('poolside-post.webp')} loading="lazy" decoding="async" alt="Maya beside the sunlit pool"/>}</div>
    ...
    <div className="ig-bottom" aria-hidden="true"><House size={24} weight="fill"/><MagnifyingGlass size={24}/><PlusSquare size={24}/><VideoCamera size={24}/><img src={media('avatar.webp')} loading="lazy" decoding="async" alt=""/></div>
  </article>;
}
