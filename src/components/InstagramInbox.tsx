...
export default function InstagramInbox(){
 ...
 return <section className="inbox-page" aria-label="Illustrative audience messages">
  ...
  <div className="inbox-notes" aria-hidden="true"><div className="inbox-note"><span className="note-bubble">Your note</span><span className="note-avatar"><img src={media('avatar.webp')} loading="lazy" decoding="async" alt=""/><i><Plus size={13}/></i></span><span>Your note</span></div>{conversations.slice(0,3).map((person,i)=><div className="inbox-note" key={person.name}><span className="note-bubble">{['Need this 🤍','So good!','New favourite'][i]}</span><span className="fan-avatar" style={{'--avatar-color':person.color} as CSSProperties}><img src={active ? media(`inbox/${person.avatar}`) : ''} width="192" height="192" loading="lazy" decoding="async" alt=""/></span><span>{person.name}</span></div>)}</div>
  ...
  <div className="inbox-messages"><div className="conversation-list">{conversations.map((person,i)=><article className={`conversation${person.key?' conversation-key':''}`} key={person.name} data-conversation={i}><span className="fan-avatar" style={{'--avatar-color':person.color} as CSSProperties} aria-hidden="true"><img src={active ? media(`inbox/${person.avatar}`) : ''} width="192" height="192" loading="lazy" decoding="async" alt=""/></span><div className="conversation-text"><div><strong>{person.name}</strong><span>{person.time}</span></div><p>{person.message}</p></div><i className="unread-dot" aria-label="Unread"/><Camera className="conversation-camera" size={24} aria-hidden="true"/></article>)}</div></div>
 </section>;
}
