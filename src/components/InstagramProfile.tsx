...
export function ProfileNavigation(){
 return <nav className="profile-nav" aria-hidden="true"><House size={24}/><MagnifyingGlass size={24}/><PlusSquare size={24}/><VideoCamera size={24}/><img src={media('avatar.webp')} alt=""/></nav>;
}
  ...
  return <section className="profile-layer" aria-label="Maya's illustrative Instagram profile">
    <div className="profile-details">
      ...
      <div className="profile-identity">
        <span className="profile-avatar"><img src={media('avatar.webp')} alt="Maya"/></span>
    ...
    <div className="profile-highlights" aria-hidden="true">{portraits.slice(0,4).map(p=><div key={p.file}><img src={media(`profile/${p.file}.webp`)} alt=""/><span>{p.label}</span></div>)}<div><span className="highlight-new"><Plus size={22}/></span><span>New</span></div></div>
    ...
    <div className="profile-grid"><div className="featured-target">{staticView && <img className="static-post-photo" src={media('poolside-post.webp')} alt="Maya’s featured poolside photograph"/>}</div>{portraits.map(p=><img key={p.file} src={media(`profile/${p.file}.webp`)} alt={p.alt}/>)}</div>
